...
describe('AutoCompleteReducer', () => {
  ...
  it('filter options with not initOptions', () => {
    const newState = AutoCompleteReducer(undefined, {type: 'filter', propertyFilter: 'name', value: 'two'});
    expect(newState).toEqual({})
  })

  it('filter options with valid options', () => {
    let newState = AutoCompleteReducer({initOptions: testOptions}, {type: 'filter', propertyFilter: 'name', value: 'two'});
    expect(newState.options).toEqual([{name: 'two'}])

    newState = AutoCompleteReducer({initOptions: testOptions}, {type: 'filter', propertyFilter: 'name', value: 't'});
    expect(newState.options).toEqual([{name: 'two'}, {name: 'three'}])
  })

  it('clear indexSelector and showOptions', () => {
    let newState = AutoCompleteReducer({initOptions: testOptions, indexSelector: 2, showOptions: true}, {type: 'clear'});
    expect(newState).toEqual({initOptions: testOptions, indexSelector: null, showOptions: false})
  })
  ...
  describe('update indexSelector', () => {
    ...
    it('sweepUp with valid indexSelector and valid options', () => {
      const newState = AutoCompleteReducer({options: testOptions, indexSelector: 1}, {type: 'sweepUp'});
    ...
    it('sweepDown with valid indexSelector and valid options', () => {
      const newState = AutoCompleteReducer({options: testOptions, indexSelector: 1}, {type: 'sweepDown'});
